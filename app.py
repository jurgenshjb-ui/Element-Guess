...
# Your custom periodic-table component (Streamlit Component)
...
# =========================================================
APP_VERSION = "v1.4.2"
GITHUB_URL = "https://github.com/jurgenshjb-ui/Element-Guess"

WIKI_BASE = "https://en.wikipedia.org/wiki/"
RSC_SEARCH = "https://www.rsc.org/periodic-table/element/"  # atomic number path
WEBELEMENTS_BASE = "https://www.webelements.com/"           # symbol path is typical, varies

# =========================================================
# Production / Debug gating (Streamlit Cloud -> App settings -> Secrets)
# STREAMLIT_ENV = "prod"   (optional)
# DEBUG = "true"           (optional; enables debug UI even in prod)
# =========================================================
IS_PRODUCTION = (
    os.environ.get("STREAMLIT_ENV") == "prod"
    or str(st.secrets.get("STREAMLIT_ENV", "dev")).lower() == "prod"
)
...
class Element:
    ...
    symbol: str
    atomic_number: int
    category3: str                   # metal/nonmetal/metalloid
    group: Optional[int]
    period: Optional[int]
...
def origin_natural_vs_synthetic(e: Element) -> str:
    # Rough but useful: mostly synthetics >=93, plus Tc/Pm
    if e.atomic_number >= 93 or e.atomic_number in (43, 61):
        return "synthetic"
    return "natural"
...
def load_elements(path: str = "PeriodicTableJSON.json") -> List[Element]:
    ...
    for r in raw:
        ...
        # Boiling point (K) / Melting point (K): accept either *_K or legacy boil/melt fields
...
# Tie-break order only (max-pool selection decides primary)
CLUE_ORDER = [
    "category3",
    "group_family",
    "block",
    ...
    "metal_group", # only after category metal
    "period",      # gated (anti-spoiler)
    "boil_split",  # guess>=3 and mutually exclusive with melt_split
    "melt_split",
]
...
def _temp_value_K(e: Element, kind: str) -> Optional[float]:
    return e.boiling_point_K if kind == "boil" else (e.melting_point_K if kind == "melt" else None)
...
# - Eligible from guess >= 3
# - Only one of boil/melt appears per game (mutual exclusion)
# - Require at least 70% known among candidates (avoid "unknown" chaos)
# =========================================================
...
def debug_unrevealed_keys(revealed: Dict[str, str]) -> List[str]:
    # Debug ignores gating EXCEPT mutual exclusion of boil/melt
    keys = [k for k in CLUE_ORDER if k not in revealed]
    if "boil_split" in revealed and "melt_split" in keys:
        keys.remove("melt_split")
    if "melt_split" in revealed and "boil_split" in keys:
...
# =========================================================
# Clue selection (entropy-free): choose clue that leaves the largest
# remaining pool for the secret value, while still narrowing (0 < cnt < N)
# =========================================================
def choose_next_clue_max_pool(
    ...
    for k in remaining_keys:
        if k in ("boil_split", "melt_split"):
            ...
            v = f"{op}|{int(x)}"
            cand = (k, v, cnt, CLUE_ORDER.index(k))
        else:
            secret_value = prop(secret, k)
            cnt = sum(1 for e in current_candidates if prop(e, k) == secret_value)
            if not (0 < cnt < current_n):
                continue
            cand = (k, secret_value, cnt, CLUE_ORDER.index(k))

        if best is None:
            best = cand
        else:
            _, _, best_cnt, best_ord = best
            _, _, cnt, ord_idx = cand
            if cnt > best_cnt:
                best = cand
            elif cnt == best_cnt and ord_idx < best_ord:
                best = cand
...
def reveal_next_clue(secret: Element, elements: List[Element], attempt: int) -> None:
    ...
    if k not in st.session_state.revealed:
        ...
        _record_candidates_checkpoint(elements)


# =========================================================
# Starter clue (informational-only): shown at start for Easy/Normal
# (Does NOT filter candidates and does not affect clue selection.)
# =========================================================
def starter_clue_text(secret: Element) -> str:
    # simple, fun facts that don't spoil too hard
    pieces = []
    pieces.append(f"**Block:** {block_of(secret)}")
    pieces.append(f"**State:** {secret.state}")
    if secret.is_noble_gas:
        pieces.append("**Noble gas:** Yes")
    if is_lanthanoid_or_actinoid(secret):
        pieces.append("**Series:** Lanthanoid/Actinoid")
    return " • ".join(pieces)
...
# =========================================================
# Candidate history checkpoints for testing
# =========================================================
...
def _record_candidates_checkpoint(elements: List[Element]):
...
def _record_test_run(secret_atomic: int):
    ...
    arr.append(rec)
    st.session_state.test_metrics[secret_atomic] = arr[-3:]
...
def stats_panel():
    stats = load_stats()
    with st.expander("📊 Stats (this session)", expanded=False):
        if stats["games_played"] == 0:
            st.caption("No games played yet in this session.")
            return
...
# =========================================================
# Definitions panel (moved under table)
# =========================================================
...
- **X** is chosen from remaining candidates and rounded to nearest **100 K**
"""
        )
...
def how_to_play():
    with st.expander("❓ How to play", expanded=False):
        st.markdown(
            """
**Goal:** Guess the hidden element in the allowed number of guesses.
...
- **Hard:** invalid guesses are rejected (no guess consumed)

**Clues**
- A few clues are revealed as you play. Each clue should narrow the set of valid candidates.
"""
        )


# =========================================================
# Tiles + statuses + animations + block outlines
# =========================================================
...
def make_tiles(
    ...
    def tile_dict(e: Element, row: str, pos: Optional[int], group: Optional[int], period: Optional[int]) -> dict:
        return dict(
            ...
            # used for block outlines in TSX (optional)
            block=block_of(e),
        )
...
# =========================================================
# End-game summary (Did you know + facts grid + links)
# =========================================================
...
def render_endgame_summary(secret: Element, revealed: Dict[str, str], status: str):
    ...
    def row(prop_name: str, value: str, key: str) -> dict:
        # for boil/melt we consider revealed if corresponding split is revealed
        if key == "boil_point":
            ok = ("boil_split" in revealed)
        elif key == "melt_point":
            ok = ("melt_split" in revealed)
        else:
            ok = (key in revealed)
        return {"Property": prop_name, "Value": value, "Status": ("✅ Revealed" if ok else "❌ Not revealed")}

    core = [
        row("Category", secret.category3, "category3"),
        row("Major group family", group_family(secret), "group_family"),
        row("Electron block", block_of(secret), "block"),
        row("State at room temp", secret.state, "state"),
        row("Atomic number range", atomic_band(secret.atomic_number), "band"),
        row("Noble gas", "Yes" if secret.is_noble_gas else "No", "noble_gas"),
    ]
    advanced = [
        row("Radioactive", "Yes" if is_radioactive(secret) else "No", "radioactive"),
        row("Origin", origin_natural_vs_synthetic(secret), "origin"),
        row("Metal group", metal_group(secret), "metal_group"),
        row("Period", str(secret.period), "period"),
    ]
    thermal = [
        row("Melting point (K)", format_K(secret.melting_point_K), "melt_point"),
        row("Boiling point (K)", format_K(secret.boiling_point_K), "boil_point"),
    ]
    ...
    with c1:
        ...
        subtxt = "rgba(255,255,255,0.85)" if status == "won" else "rgba(255,255,255,0.75)"
        st.markdown(
            f"""
    ...
    with c2:
        ...
        with st.expander("Thermal properties", expanded=False):
            st.dataframe(thermal, use_container_width=True, hide_index=True)
...
def start_new_game(elements: List[Element], game_mode: str):
    ...
    st.session_state.max_guesses = int(st.session_state.get("max_guesses", 8))
...
def main():
    ...
    with st.sidebar:
        ...
        if SHOW_DEBUG_UI:
            ...
            if st.session_state.debug_enabled:
                ...
                # Max guesses: preset dropdown + custom box
                presets = [6, 7, 8, 9, 10, 12, 15]
                current = int(st.session_state.get("max_guesses", 8))
                preset_label = f"{current}"
                options = [str(p) for p in presets] + ["Custom"]
                idx = options.index(preset_label) if preset_label in options else options.index("Custom")
                chosen = st.selectbox("Max guesses (preset)", options, index=idx)

                if chosen == "Custom":
                    mg = st.number_input("Max guesses (custom)", min_value=3, max_value=30, value=current, step=1)
                else:
                    mg = int(chosen)
                if int(st.session_state.get("max_guesses", 8)) != int(mg):
                    st.session_state.max_guesses = int(mg)
                    # If game already started, keep current run but apply next restart
                    st.caption("Applies immediately on Restart (or next new game).")
                ...
                options2 = ["— none —"] + [
                    f"{by_atomic[n].name} ({by_atomic[n].symbol}) #{n}" for n in targets if n in by_atomic
                ]
                current_idx = 0
                ...
                choice = st.selectbox("Force target element (for testing)", options2, index=current_idx)
                if choice == "— none —":
                    st.session_state.debug_target_override_atomic = None
                else:
                ...
                if st.button("↩️ Undo last guess", disabled=not can_undo, use_container_width=True):
                    undo_snapshot()
    ...
    if st.session_state.game_mode == "Daily":
        today = dt.date.today()
        st.info(f"📅 Today: {today:%A, %d %B %Y} • Day {day_number()} • Fingerprint: {daily_fingerprint(st.session_state.secret.atomic_number)}")

    # Effective secret (debug override)
    if st.session_state.get("debug_enabled") and st.session_state.get("debug_target_override_atomic"):
    ...
    # Starter clue (informational)
    if st.session_state.status == "playing" and st.session_state.attempt == 0 and st.session_state.difficulty in ("Easy", "Normal"):
        st.info(f"✨ **Starter clue:** {starter_clue_text(secret)}")
    ...
    st.write(f"🎯 **Guesses used:** {st.session_state.attempt}/{int(st.session_state.get('max_guesses', 8))}")

    # New clues ABOVE the periodic table
    if st.session_state.revealed:
        st.subheader("Revealed clues")
        for i, k in enumerate(st.session_state.revealed_order, start=1):
            if k not in st.session_state.revealed:
                continue
    ...
    # Debug analytics in main area (restored)
    ...
    if st.session_state.invalid_atomic is not None:
        st.session_state.invalid_atomic = None
    if st.session_state.last_guess_atomic is not None:
        st.session_state.last_guess_atomic = None
    ...
    if click and st.session_state.status == "playing":
        ...
        if nonce is not None and nonce == st.session_state.get("last_click_nonce"):
            return
        ...
        # Hard: invalid guess rejected (no guess consumed)
        ...
        # lose?
        if st.session_state.attempt >= int(st.session_state.get("max_guesses", 8)):
